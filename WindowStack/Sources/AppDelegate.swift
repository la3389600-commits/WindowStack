    ...
    private func loadPreferences() {
        if let data = UserDefaults.standard.data(forKey: configKey),
           var config = try? JSONDecoder().decode(PanConfig.self, from: data) {
            if config.fadeStyleRevision < PanConfig.currentFadeStyleRevision {
                let fresh = PanConfig()
                config.switchFadeIntensity = fresh.switchFadeIntensity
                config.switchFadeDuration = fresh.switchFadeDuration
                config.switchFadeBrightness = fresh.switchFadeBrightness
                config.fadeStyleRevision = PanConfig.currentFadeStyleRevision
                saveConfig(config)
            }
